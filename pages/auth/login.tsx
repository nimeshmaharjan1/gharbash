...
const SignUp: NextPage<{ session: any }> = ({ session }) => {
    ...
    const signInWithGoogle = async () => {
        setIsLoading(true);
        await signIn('google', {
            callbackUrl: 'http://localhost:3000/homes/create',
            redirect: true,
        });
    };
    return (
                ...
                {showConfirm ? (
                    <div className="card-body">
                        ...
                        <div className="p text-center text-sm md:text-[15px] opacity-80">
                            We emailed a magic link to <span className="font-bold text-primary">{submittedEmail}</span>. Check your inbox
                            and click the link in the email to login or sign up.
                        </div>
                    </div>
                ) : (
                    <>
